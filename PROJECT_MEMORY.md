...
- Current reconciliation classification: **YES** — listener-at-event-time routing plus platform feedback transport/timing changes can alter perceived audio positioning or cue delivery. A final owner check is therefore required on the final exact Android candidate. See `docs/OWNER_RELEASE_SAFETY_CHECKLIST.md` for the separate owner-only signing-safety checklist.
