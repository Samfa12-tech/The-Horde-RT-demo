...
- Current Vulkan-enabled host configurations have thirteen CTests; clean merged-main Host run `run-20260823-202515` passed 13/13 in fresh Debug and Release, 13 deterministic Windows captures, Android Debug/unsigned Release/lint, packaging/licence checks, shader checks, and evidence hashes. The connected `SM-S948B` retry run `run-20260823-201800` separately passed the six-checkpoint route, replay, 13 captures, and Home/resume after an ADB-daemon interruption in the enclosing Full run. The portable CI lane has nine Vulkan-disabled tests and does not prove hardware RT.
