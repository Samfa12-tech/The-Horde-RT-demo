...
- The published complete showcase route remains Windows- and Android-device-validated at one active skinned enemy. Historical exact two-skeleton commit `b3428a7` separately passed its `SM-S948B` gate: six 75% checkpoint medians of 10.589 / 12.139 / 9.246 / 8.888 / 11.060 / 19.735 ms at thermal status 0, plus 13 captures, replay, and Home/resume. The owner subsequently reported the hands-on two-enemy experience "feels fine" on that still-installed exact candidate. This evidence does not prove later `daa5892` parry or `b9212e7+` press-edge/reconciliation source. See `docs/HORDE_SHOWCASE_ANDROID_VALIDATION_2026-07-17.md` and `docs/TWO_SKELETON_COMBAT_ANDROID_VALIDATION_2026-08-12.md`.
...
- Current Vulkan-enabled host configurations have twelve CTests; the portable CI lane has eight Vulkan-disabled tests and does not prove hardware RT. Final revalidation of current `b9212e7+` source remains pending.
