...
- The published complete showcase route remains Windows- and Android-device-validated at one active skinned enemy. Historical exact two-skeleton commit `b3428a7` passed its own gate and owner hands-on check. Current exact runtime/authority candidate `4a4d360` separately passed strict ASTC, honest RT presentation, replay, 13 captures, Home/resume, a cooled six-checkpoint 75% gate (lich 19.695 ms, `PASS - LOW HEADROOM`), and matched lich GPU-timing A/B (19.686/19.587 ms, 0.505%). The owner reports that event-time spatial audio sounds good on the exact installed candidate; exact-candidate haptic confirmation remains pending and must not be inferred from automation. See `docs/HORDE_SHOWCASE_ANDROID_VALIDATION_2026-07-17.md`, `docs/TWO_SKELETON_COMBAT_ANDROID_VALIDATION_2026-08-12.md`, and `docs/CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`.
...
- The 75% performance hard gate is strictly below 20.000 ms at every required checkpoint. From 18.500 ms inclusive to below 20.000 ms, report `PASS - LOW HEADROOM`; at or above 20.000 ms, report `FAIL`. Do not weaken RT, lower the gate, or automatically reduce resolution to remove a warning.
- The older renderer-foundation candidate failed the lich gate at 23.604 ms during an unmatched run that heated AP 31.9 to 51.1 C. Matched cooled exact-APK A/B later measured 19.497 ms with GPU timestamps and 19.268 ms without, only 1.188% apart, so timestamp overhead was not the cause. Treat the older failure as superseded thermal/run-order evidence, not the current candidate result.
- Two simultaneous skeletons are the hard development ceiling. Do not add a third/fourth enemy, broader AI, block/dodge, or unrelated gameplay. Keep the lich singular. Automated phone and broad owner hands-on promotion gates are passed; the brief owner verdict does not independently certify exact cue-by-cue audio/haptic distinction.
- Current Vulkan-enabled host configurations have twelve CTests; canonical clean Host run `run-20260821-170907` passed 12/12 in fresh Debug and Release, 13 captures, Android Debug/unsigned Release/lint, packaging/licence checks, shader checks, and evidence hashes. The portable CI lane has eight Vulkan-disabled tests and passed on PR #14, but does not prove hardware RT.
