...
- The published complete showcase route remains Windows- and Android-device-validated at one active skinned enemy. Historical exact two-skeleton commit `b3428a7` passed its own gate and owner hands-on check. The current reconciliation branch separately passed strict ASTC, honest RT presentation, replay, 13 captures, Home/resume, and matched lich GPU-timing A/B (19.686/19.587 ms, 0.505%). The owner reports that event-time spatial audio sounds good on the exact installed candidate; exact-candidate haptic confirmation remains pending and must not be inferred from automation. See `docs/HORDE_SHOWCASE_ANDROID_VALIDATION_2026-07-17.md`, `docs/TWO_SKELETON_COMBAT_ANDROID_VALIDATION_2026-08-12.md`, and `docs/CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`.
