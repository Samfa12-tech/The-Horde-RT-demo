...
- Sanitized exact Meshy settings/task IDs and accepted/rejected decisions are tracked in `assets/models/props/provenance/task-7/`. The external icon is not a repository/distributable file; the final programme audit later re-opened the owner package, visually re-inspected both supplied references, and independently confirmed their hashes.
- Historical boundary: `SM-S948B` was unavailable during Task 7. The later integrated Task 9/current-candidate device evidence is recorded in `FIRE_PBR_REWARD_LANTERN_PLAYER_UPGRADE_VALIDATION_2026-08-30.md`; it must not be inferred from this earlier Host-only repair.
