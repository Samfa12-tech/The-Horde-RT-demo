...
The Windows and Android alphas are public on separate itch channels. Samfa12.com links to itch rather than hosting a second copy. Anonymous checks returned HTTP 200 for the project and purchase pages and exposed both versioned downloads.
...
- [x] Butler Windows channel published and verified after the documentation refresh: `samfa12/the-horde:windows-x64`, upload `#18339908`, build `#1797811`, user version `0.1.0-alpha.1`.
...
- [x] Butler Android channel published and verified: `samfa12/the-horde:android`, upload `#18341739`, build `#1797750`, user version `0.1.0-alpha.1`.
...
- [ ] Exercise Resume, Restart, Settings, Diagnostics, Quit, F1/F2, fullscreen, resize, minimize/restore, swing SFX, and look sensitivity.
- [x] Confirm `RT scene presented: yes` in the packaged `reports/` output.
- [ ] Capture final entry/pause, live scene, settings, and diagnostics evidence at 100% and 150% Windows scaling.
...
- [x] Upload the Windows x64 zip after clean extraction validation (`samfa12/the-horde:windows-x64`, build `#1797811`).
...
- [x] Guarded Butler workflow published the non-debug, hash-matched APK to `android`: upload `#18341739`, build `#1797750`.
...
- [ ] Complete the extended Windows interaction sweep and 100%/150% scaling captures recorded above.
- [ ] Add the Credits block from `release/SAMFA12_SITE_COPY.md` to the live itch description, including explicit Hotstrike Studio and Meshy attribution; the repository link alone is weaker than direct public credit for the Android APK.
- [ ] Ensure itch's AI disclosure includes the Meshy-assisted graphics/asset contribution if the available category supports it.
