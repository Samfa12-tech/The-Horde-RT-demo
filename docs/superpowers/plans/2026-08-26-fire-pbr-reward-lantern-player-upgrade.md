...
- [x] Bind dropped-torch gameplay, active character placement, corridor geometry, and the reusable Task 8 reward transform to the neutral `ShowcaseRoute.h` `kRouteFloorWorldY=-0.95` authority; prove shared settle/rest/stage behavior, full transformed AABBs, floor contact, base/lid clearance, exact `RewardLanternHingeSocket` composition, and both final checkpoint frusta.
