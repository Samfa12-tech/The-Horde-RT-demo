...
- classifies each median-of-window-averages against descriptive 16.667/20.000/33.333 ms reference lines (approximately 60/50/30 FPS), records thermal context and the Adreno GPU thermal power level when available, and does not turn 20 ms into an automatic product failure;
...
`Disabled` removes the per-frame query reset, timestamp writes, and result readback only. Native `vkCmdTraceRaysKHR`, ray-query shading, swapchain presentation, and render scale are unchanged. Each run hashes the local and installed APK and refuses an artifact mismatch; schema-7 summaries record commit/dirty state, embedded raygen hash, device serial, OS build fingerprint, Vulkan GPU/driver identity, FPS-reference bands, warnings, and GPU thermal power level where exposed. The comparator requires one enabled and one disabled run with matching provenance and honest timing-row metadata, AP/SKIN/BAT starting temperatures within 2 C by default, and no enabled-versus-disabled regression above the 15% investigation threshold. Historical schema-4+ evidence remains readable, with schema 4 explicitly labelled as lacking serial/fingerprint provenance.

The standard order is intentionally one sustained app process. Do not force-stop between checkpoints and then describe the results as sustained behavior: `SM-S948B` provides a temporary fresh-process GPU boost even when already warm. Cooled or isolated runs are useful diagnostic controls, but the sustained route is the primary player-facing performance report.
...
The original shared checkpoint builders and route replay pass Windows host smoke tests. Android Debug and Release APKs compile with the automation enabled only for Debug. Historical exact two-enemy commit `b3428a7` passed its then-current six-checkpoint gate, matched GPU-timing A/B, replay/captures, and Home/resume. Exact `daa5892` later passed functionality and recorded a 20.246 ms warm lich median. Final exact reconciliation runtime commit `547d89d` passed the complete clean Full gate, exact installed-APK matching, strict ASTC, honest presentation, 13-waypoint replay, 13 captures, and Home/resume; its sustained last-in-order lich measured 23.069 ms in the descriptive 30-50 FPS band at GPU thermal power level 2. Earlier exact `88868f4` attribution showed flat process/graphics memory alongside Samsung GPU frequency/power-level throttling, motivating the evidence-first reporting policy above. The owner gave the exact final candidate a broad audio/haptic pass and explicitly observed its stagger-back/death sequence; precise spatial propagation remains automated-contract evidence. See `ANDROID_SHOWCASE_AUTOMATION_VALIDATION_2026-07-17.md`, `TWO_SKELETON_COMBAT_ANDROID_VALIDATION_2026-08-12.md`, and `CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`. Owner-only release safety is in `OWNER_RELEASE_SAFETY_CHECKLIST.md`.
