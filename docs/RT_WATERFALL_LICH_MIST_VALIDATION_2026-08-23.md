...
- High uses one bounded transmission query plus one bounded reflection query. Mobile uses one bounded transmission query plus analytic environment reflection. Current development additionally shades refracted/reflected opaque hits and interface highlights with the shared scene-light visibility path; water-on-water secondary hits remain non-recursive. Off skips water candidates on the primary query while preserving all geometry and gameplay contracts.
...
## Development general-lighting follow-up - 2026-08-24

The published 0.1.5 artifacts above remain immutable. The first later correction added one visibility query to the water-only transmission helper, but subsequent fixed transport percentages still filled the player shadow. Current development removes that approximation and a later discovered double reflection: refracted and High reflected opaque hits use terminal shared material/direct-light/visibility shading, interface highlights use the same active lights, transparent filtering explicitly handles opaque BLAS candidates, accumulated path distance is preserved, and the directional moon traverses real roof/player geometry. Screen-position shadow masks are removed and water-on-water secondary hits remain non-recursive. See `WATER_TRANSMISSION_SHADOW_VALIDATION_2026-08-24.md`.

Fresh shader, host, deterministic Windows, and exact 2026-08-25 current-candidate phone evidence are recorded in `WATER_TRANSMISSION_SHADOW_VALIDATION_2026-08-24.md`. The phone candidate passed installed-byte parity, focused timing, replay, 13 captures, and Home/resume; its repeated bounded real-light cost is documented without a quality or resolution reduction. This follow-up does not alter water geometry, drench timing, audio, haptics, controls, gameplay authority, recursion depth, RT presentation, strict ASTC policy, or published packages.
