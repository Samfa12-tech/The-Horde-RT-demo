...
- Negative fixtures prove rejection of stale embedded SPIR-V, missing licences/assets, invalid ZIP root layout, immutable 0.1.1/0.1.2/0.1.3 identities, old Android version codes, and another upload on a published line before Butler is contacted.
- Future candidate packaging requires explicit `Version` and `VersionCode`, rejects the immutable published 0.1.1/0.1.2/0.1.3 lines, and requires `versionCode > 4`.
