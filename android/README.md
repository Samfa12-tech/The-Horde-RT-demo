...
The current showcase release identity is `1.5.2` with `versionCode 7`. Public candidates must be signed by the established Horde release key, retain strict ASTC routing and 16 KiB compatibility, and pass the guarded package/push verification; never replace the existing update identity with a new keystore.

The exact signed 1.5.2 APK is published as itch build `#1913192`; SHA-256 `19593f9d8902052cb54f9b989f9646ec8cad97063db5d882e1487dd56a671182`. The established certificate, version, credits, strict package layout, waterfall asset, static C++ runtime, APK/ELF 16 KiB alignment, and native libraries passed. No ADB device was connected at publication, so this exact signed APK has not received install, pullback, strict-ASTC runtime, honest-presentation, or Home/resume release smoke. The exact 0.1.5 signed package remains the latest public APK directly validated on `SM-S948B`. See `../docs/SHOWCASE_ALPHA_1_5_2_RELEASE_VALIDATION_2026-08-25.md`.
