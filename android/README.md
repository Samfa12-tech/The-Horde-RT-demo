...
That historical exact candidate is clean commit `b3428a7`; it does not validate the later animation/parry work. Exact clean `daa5892` passed functionality but missed its warm lich gate. The owner found parry timing good and requested touch-down dispatch. Current exact runtime/authority candidate `4a4d360` uses press-down parry plus listener-at-event-time routing and passes strict ASTC, honest presentation, replay, 13 captures, Home/resume, a cooled six-checkpoint 75% gate, matched GPU-timing A/B, and the complete clean Host foundation. The owner reports that audio sounds good on the exact installed candidate; exact-candidate haptic confirmation remains pending. Future manual owner audio/haptic checks are change-triggered rather than automatic per milestone. See `../docs/ANIMATION_COMBAT_PARRY_SLICE_2026-08-13.md`, `../docs/CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`, and `../docs/OWNER_RELEASE_SAFETY_CHECKLIST.md`.
