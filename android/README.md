...
The debug build exposes thirteen deterministic native checkpoints and a 13-waypoint route replay. Run the standard six-checkpoint 75% timing, collision, strict-ASTC, and honest-presentation pass from the repository root. Every 75% median must remain strictly below 20.000 ms; 18.500 ms inclusive to below 20.000 ms passes with a low-headroom warning, while 20.000 ms or above fails:
...
That historical exact candidate is clean commit `b3428a7`; it does not validate the later animation/parry work. Exact clean `daa5892` passed strict ASTC, honest presentation, all six state checks, replay, 13 captures, and Home/resume, but its warm 75% lich median was 20.246 ms, so it did not pass the complete strict performance gate. The owner found the parry timing good and requested touch-down dispatch. Current `b9212e7+` source uses press-down parry and reconciliation listener-at-event-time changes; it awaits final host and exact-device validation. Manual owner audio/haptic checks are change-triggered rather than automatic per milestone; this reconciliation is **YES** because the listener route changes. See `../docs/ANIMATION_COMBAT_PARRY_SLICE_2026-08-13.md` and `../docs/OWNER_RELEASE_SAFETY_CHECKLIST.md`.
