...
The current showcase release identity is `0.1.4-alpha.1` with `versionCode 5`. Public candidates must be signed by the established Horde release key, retain strict ASTC routing and 16 KiB compatibility, and pass the guarded package/push verification; never replace the existing update identity with a new keystore.

The exact signed 0.1.4 APK is published as itch build `#1903587`; SHA-256 `ff549c2ab1c68fe7a36d4e45c966e664a5ec7e3f19fb8f416b7a6e8971db2e55`. The established certificate, version, credits, strict package layout, static C++ runtime, APK/ELF 16 KiB alignment, and native libraries passed. The phone was disconnected, so this exact signed APK has not yet received a release-install smoke; the byte-matched final Debug runtime remains the applicable device evidence. See `../docs/SHOWCASE_ALPHA_0_1_4_RELEASE_VALIDATION_2026-08-22.md`.
