...
The animation-owned combat/parry candidate at exact commit `daa5892` passed functional `SM-S948B` checks and recorded a 20.246 ms warm lich median under the then-current gate. The owner found parry timing good. The current reconciliation branch publishes Android parry on press-down and separately passes strict ASTC, honest presentation, replay, 13 captures, Home/resume, matched GPU-timing A/B, and the complete clean Host foundation. A successful parry keeps the attacker stationary in gameplay while the renderer procedurally carries the existing Attack clip from contact toward recovery with a bounded lean/recoil. The owner reports that spatial audio sounds good on the exact installed candidate; exact-candidate haptic confirmation remains pending. Performance now uses sustained evidence and descriptive 60/50/30 FPS bands rather than a hard 20 ms rule. No renderer slot, BLAS, pose bucket, asset, or public-release identity was added. See `docs/ANIMATION_COMBAT_PARRY_SLICE_2026-08-13.md` and `docs/CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`.
