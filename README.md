...
The current unsigned development programme adds imported PBR sword/torch/chest/reward-lantern assets, fixed-capacity generic RT material metadata, coherent world-space fire and coloured light, bounded reusable dielectric glass, shared held-item sockets, a post-lich chest/reward interaction, deterministic physical lantern swing, a downward/upward sword combo, a reusable skinned-player/IK foundation, and GitHub Release update alerts. Owner review keeps block arms in normal gameplay until the skinned hands/gauntlets are ready in every scenario; shadow/reflection arm polish is deferred. Exact clean Host, Windows feature-capture, and `SM-S948B` 75%/100% evidence is recorded in `docs/FIRE_PBR_REWARD_LANTERN_PLAYER_UPGRADE_VALIDATION_2026-08-30.md` and `docs/TASK_9_OWNER_CANDIDATE_VALIDATION_2026-08-30.md`. Instrumented Debug feature medians are 53.481-91.176 ms at 75% and 83.954-154.484 ms at 100%, so this is image/correctness evidence rather than a performance claim; Mobile glass remains the main measured risk. The final delayed unlock cue still needs an owner listening check. This development candidate is not signed, versioned, or published.
...
The Fire/PBR/reward-lantern development candidate passed clean-source Windows Debug and Release 31/31 Vulkan-enabled CTests, 13 standard plus 11 feature Windows captures, isolated packaged Windows launch, Android Debug/unsigned Release/lint, asset/licence/package gates, and exact `SM-S948B` 75%/100% RT/ASTC/capture/Home-resume validation. The installed Debug APK SHA-256 is `0b5a59b6e41d2c4d717eff885aaa310b7f5f1512002f6a89cb77e5989ab7edd3`; the owner reported that phone play feels good. The feature path is materially slower than the published route and finite Mobile glass-budget terminals remain, both recorded without reducing quality or scale. The final two-second unlock-sound timing still needs one owner listening check. It remains unsigned and unpublished. See `docs/FIRE_PBR_REWARD_LANTERN_PLAYER_UPGRADE_VALIDATION_2026-08-30.md` and `docs/TASK_9_OWNER_CANDIDATE_VALIDATION_2026-08-30.md`.
