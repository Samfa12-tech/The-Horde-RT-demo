...
The animation-owned combat/parry candidate at exact commit `daa5892` passed functional `SM-S948B` checks but missed its warm lich performance gate. The owner found parry timing good. Current exact runtime/authority candidate `4a4d360` publishes Android parry on press-down and separately passes strict ASTC, honest presentation, replay, 13 captures, Home/resume, the cooled six-checkpoint 75% gate, and matched GPU-timing A/B. A successful parry keeps the attacker stationary in gameplay while the renderer procedurally carries the existing Attack clip from contact toward recovery with a bounded lean/recoil. The one-time owner event-time spatial-audio/haptic baseline and canonical fresh Host rerun remain pending. No renderer slot, BLAS, pose bucket, asset, or public-release identity was added. See `docs/ANIMATION_COMBAT_PARRY_SLICE_2026-08-13.md` and `docs/CURRENT_DEVELOPMENT_BASELINE_VALIDATION_2026-08-21.md`.
