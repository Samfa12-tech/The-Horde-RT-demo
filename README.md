...
The animation-owned combat/parry candidate at exact commit `daa5892` passed functional `SM-S948B` checks (strict ASTC, honest presentation, six checkpoint states, 13 captures, replay, and Home/resume), but its warm lich median was 20.246 ms and therefore did not pass the complete strict performance gate. The owner found parry timing good. Current `b9212e7+` source publishes Android parry on press-down rather than release; it awaits final host and exact-device revalidation. A successful parry keeps the attacker stationary in gameplay while the renderer procedurally carries the existing Attack clip from contact toward recovery with a bounded lean/recoil. No renderer slot, BLAS, pose bucket, asset, or public-release identity was added. See `docs/ANIMATION_COMBAT_PARRY_SLICE_2026-08-13.md`.
