...
- Exact Android device baseline: the stable-key-signed `0.1.5` APK remains the latest public release installed and pulled back byte-for-byte on `SM-S948B`. The published `1.6.0` APK passed certificate/package checks, but no phone was connected at publication, so no exact `1.6.0` signed-device or lifecycle claim is made. The same feature runtime was accepted on the exact final Debug APK before the release-only identity/signing delta.
