...
- Diagnostic text is monospaced on a dark background overlay above a native Vulkan diagnostic `SurfaceView`.
- JNI now also exposes surface start/stop calls to drive a native swapchain diagnostic presenter.
...
- Probe text rendering remains TextView-based in an overlay while native Vulkan swapchain color output confirms app-side presentation.
