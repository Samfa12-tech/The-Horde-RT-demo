...
// A small allowlisted benchmark scenario, not access to Debug checkpoints or
// renderer tuning. Production assets, transport and held-item authority are used.
inline bool StageLanternBenchmark(simulation::GameSimulation& simulation,
                                  BenchmarkWorkload workload)
{
    if (!IsLanternBenchmark(workload)) return false;
    if (!simulation.ApplyShowcaseCheckpoint(5)) return false;
    ...
    simulation.StepFixed(input, 0.0f, simulation.Snapshot().inputPublicationSequence + 1u);
    ...
    // Resolve the gameplay hand/hinge first. Authored pendulum states stay world
    // geometry beneath that hinge; no camera-space offset or glass-only flag.
