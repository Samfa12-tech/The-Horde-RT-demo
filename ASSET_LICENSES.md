...
| Skeleton biped, merged animations v01 | Skinned GLB with 11 animation clips; embedded 4K texture | User-provided `Meshy_AI_SKM_Skeleton_Var_1_biped.zip`; merged animation GLB audited on 2026-07-10; see `assets/models/enemies/meshy/skeleton_biped_merged_animations_v01.METADATA.md` | **Pending account-plan verification.** Treat as the same Meshy license gate: paid/private output may be commercially used under Meshy terms; free output requires CC BY 4.0 attribution. Do not ship until confirmed. | User asset handoff, runtime-integrated by Codex, 2026-07-11 | One enemy is packaged with Android, CPU-skinned from `Idle_5` and refit into a dynamic RT BLAS. The first proof uses a procedural bone material; the embedded 4K texture is not uploaded yet. The sword remains separate and is not attached to `RightHand`. |
