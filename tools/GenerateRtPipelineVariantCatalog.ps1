...
function Render-Row([object]$row) { "    RtPipelineCatalogRecord{{RtInstrumentation::$($row.instrumentation), DielectricQuality::$($row.quality), RtMaterialStrategy::$($row.material)}, `"$($row.key)`", `"$($row.artifactPath)`", `"$($row.spirvSha256)`", `"$($row.includeSha256)`", $($row.words)}" }
